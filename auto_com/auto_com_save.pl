...
sub SAVE{
	...
	if ($in{'first'} eq "" || $in{'first'} =~ m/[^0-9]/ || $in{'first'} < 1 || $in{'first'} > $BMMAX_COM) { &ERR("開始位置の指定が異常です。"); }
	if ($in{'second'} eq "" || $in{'second'} =~ m/[^0-9]/ || $in{'second'} < 1 || $in{'second'} > $BMMAX_COM) { &ERR("終了位置の指定が異常です。"); }
	if ($in{'first'} > $in{'second'}) { &ERR("開始位置より終了位置のほうが小さいです。"); }
	if(length($in{'name'}) > 15 * 3) { &ERR("コマンドリストの名前が15文字以上になっています。(送信内容 : $in{name})"); }
	&CHARA_MAIN_OPEN;
	&TIME_DATA;
	...
	open(IN,"./charalog/auto_com_save/$kid/$in{'save'}.cgi");
	@COM_DATA = <IN>;
	close(IN);
	...
	for($i=$in{'first'};$i<=$in{'second'};$i++){
		($cid,$cno,$cname,$ctime,$csub,$cnum,$cend) = split(/<>/,$AUTO_COM_DATA[$i]);
		push(@NEW_COM_DATA,"$cid<>$cno<>$cname<>$ctime<>$csub<>$cnum<>$cend<>\n");
	}
	open(OUT,">./charalog/auto_com_save/$kid/$in{'save'}.cgi") or &ERR('ファイルを開けませんでした。');
	...
	open(OUT,">./charalog/auto_com_save/$kid/list.cgi") or &ERR('ファイルを開けませんでした。');
...
sub LOOK{
	...
	#データ処理&ファイルオープン
	open(IN,"./charalog/auto_com_save/$kid/$in{'save'}.cgi") or &ERR("$in{'save'}番目には保存されていません。");
	@COM_DATA = <IN>;
	close(IN);
	$mes_num = @COM_DATA;
...
sub LOAD{
	...
	for($i=$in{'first'};$i<=$in{'second'};$i++){
		if($n < $mes_num){
			...
			splice(@AUTO_COM_DATA,$i,1,"$cid<>$cno<>$cname<>$ctime<>$csub<>$cnum<>$cend<>\n");
			$n++;
		}else{
			$n=0;
			($cid,$cno,$cname,$ctime,$csub,$cnum,$cend) = split(/<>/,$COM_DATA[$n]);
...
sub NAME{
	...
	if ($in{'save'} eq "" || $in{'save'} =~ m/[^0-9]/ || $in{'save'} < 1 || $in{'save'} > 15) { &ERR("保存リストの番号指定が異常です。"); }
	if($in{'name'} eq "" || length($in{'name'}) > 15) { &ERR("名前が入力されていないか15文字以上になっています。(入力内容 : $in{name})"); }
	&CHARA_MAIN_OPEN;
	&TIME_DATA;
