...
sub CHECK_COM{
	...
	if ($mmonth eq "1" || $mmonth eq "7") {
		...
		while ($file = readdir $dh) {
			if ($file =~ /\.cgi/i) {
        my $chara_data = open_data("$dir/$file");
				my ($kid,$kpass,$kname,$kchara,$kstr,$kint,$klea,$kcha,$ksol,$kgat,$kcon,$kgold,$krice,$kcex) = split /<>/, $chara_data->[0];
				$cou_num[$kcon]++;
				$cex_total[$kcon]++;
			}
		}
	...
	while ($file = readdir $dh) {
		...
		if ($file =~ /\.cgi/i) {
      my $chara_data = open_data("$dir/$file");
      suport_expand_my_chara_to_global_variables($chara_data->[0]);
        ...
        my $stop_count = $stop_update_date->[0];
        ...
        $kdate += $stop_count < 18 ? $TIME_REMAKE : $TIME_REMAKE * 55;
            ...
            my $current_town = $town_model->get($kpos);
            my $move_town    = $town_model->get($cnum);

            if ( $current_town->can_move($move_town) ) {
							$kpos = $cnum;
							$klea_ex++;
							$kcex += 28;
							$ksub1 = "$kstr_ex,$kint_ex,$klea_ex,$kcha_ex,$ksub1_ex,$ksub2_ex,";
					  ...
					  $ksub2=0;
						...
						} elsif ((my $average_loyalty = Jikkoku::Util::average_loyalty($kcon)) > 50) {
              &K_LOG("$mmonth月:国民の忠誠度平均が50以下ではありません。（平均：$average_loyalty）");
						} else {
						...
						$kcon = 0;
							...
							for($i=$randy;$i<$BM_Y;$i++){
								for($j=$randx;$j<$BM_X;$j++){
									if($BM_TIKEI[$i][$j] ne "" && $BM_TIKEI[$i][$j] != 18 && $BM_TIKEI[$i][$j] != 22){
									$hanran_x = $j;
									$hanran_y = $i;
									$count = 2;
									}
...
sub SALARY {
	...
	foreach(@TOWN_DATA){
		...
		if($z2con eq $kcon){
			if($mmonth eq "1"){
				$ksal += int($z2syo * 14 * $z2num / 10000);
			}elsif($mmonth eq "7"){
				$ksal += int($z2nou * 14 * $z2num / 10000);
			}
		}
	}
