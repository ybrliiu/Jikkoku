...
sub GET_SOL2 {
	...
	&CHARA_MAIN_OPEN;
	&TIME_DATA;
	if($kclass < $SOL_LANK[$in{'type'}]){
	&ERR("階級値が足りません。");
	}

	$cost = ($SOL_PRICE[$in{'type'}]*$in{'num'});
	...
	if($in{'no'} eq "all"){
		while(@NEW_COM_DATA < $MAX_COM){
			push(@NEW_COM_DATA,"10<><>$SOL_TYPE[$in{'type'}]徴兵（$in{'num'}人：$cost\G）<>$tt<>$in{'type'}<>$in{'num'}<>$in{'gat'}<>\n");
		}
		$no = $in{'no'};
	}else{
		foreach(@COM_DATA){
	...
	print <<"EOM";
<CENTER><hr size=0><h2>NO:$noに$mes$SOL_TYPE[$in{'type'}]徴兵（$in{'num'}人：$cost\G）を入力しました。</h2><p>
