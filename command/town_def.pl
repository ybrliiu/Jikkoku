...
sub TOWN_DEF {
	...
	elsif($in{'zahyou'} eq ""){&ERR("座標が入力されていません。");}
	elsif($in{'zahyou'} !~ /,/){&ERR("フォームが、不正な値です。");}
	&CHARA_MAIN_OPEN;
	&TIME_DATA;

	($x,$y) = split(/,/,$in{'zahyou'});
	if($x =~ m/[^0-9]/ || $y =~ m/[^0-9]/){&ERR("フォームが不正な値です。");}
	...
	if($in{'no'} eq "all"){
		while(@NEW_COM_DATA < $MAX_COM){
			push(@NEW_COM_DATA,"$in{'mode'}<><>城の守備$mess<>$tt<>$x<>$y<><>\n");
		}
		$no = $in{'no'};
	}else{
		foreach(@COM_DATA){
	...
	print <<"EOM";
<CENTER><hr size=0><h2>NO:$noに城の守備$messを入力しました。</h2><p>
