#_/_/_/_/_/_/_/_/_/_/#
#        削除        #
#_/_/_/_/_/_/_/_/_/_/#

sub SAKUJYO {

	if($in{'no'} eq ""){&ERR("NO:が入力されていません。");}
	if($in{'no'} eq "all"){&ERR("削除にALL選択はできません。");}
	&CHARA_MAIN_OPEN;
	&TIME_DATA;


	open(IN,"./charalog/command/$kid.cgi");
	@COM_DATA = <IN>;
	close(IN);

	$mes_num = @COM_DATA;
	if($mes_num > $MAX_COM) { pop(@COM_DATA); }


	#削除するのにしるし
	@NEW_COM_DATA=();$i=0;
	foreach(@COM_DATA){
	($cid,$cno,$cname,$ctime,$csub,$cnum,$cend) = split(/<>/);
	$ahit=0;
		foreach(@no){
			if($i eq $_){
				$ahit=1;
				push(@NEW_COM_DATA,"65<><>削除<>$tt<><><><>\n");
				$lno = $_ + 1;
				$no .= "$lno,";
			}
		}
		if(!$ahit){
			push(@NEW_COM_DATA,"$_");
		}

	$i++;
	}


#削除
	$i=0;
	$ahit=0;
	while($MAX_COM > $i){
	($cid,$cno,$cname,$ctime,$csub,$cnum,$cend) = split(/<>/,$NEW_COM_DATA[$i]);

		if($cid eq "65"){
		splice(@NEW_COM_DATA,$i,1) or &ERR('削除できませんでした。');
		push(@NEW_COM_DATA,("0<><>何もしない<>$tt<><><><>\n")) or &ERR('挿入エラー');
		$ahit++;
		}else{
		$i++;
		}
	}


	open(OUT,">./charalog/command/$kid.cgi") or &ERR('ファイルを開けませんでした。');
	print OUT @NEW_COM_DATA;
	close(OUT);

	&HEADER;

	print <<"EOM";
<CENTER><hr size=0><h2>NO:$noを削除しました。</h2><p>
<form action="$FILE_STATUS" method="post">
<input type=hidden name=id value=$kid>
<input type=hidden name=pass value=$kpass>
<input type=hidden name=mode value=STATUS>
<input type=submit value="ＯＫ"></form></CENTER>
EOM

	&FOOTER;

	exit;

}
1;
