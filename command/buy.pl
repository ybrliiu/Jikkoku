...
sub BUY {
  ...
  my $MAX_TRANSACTION_VALUE = 30000;
	...
	$get_sol1 = $kgold;
	$get_sol2 = $krice;
	if($get_sol1 > $MAX_TRANSACTION_VALUE){
		$get_sol1 = $MAX_TRANSACTION_VALUE;
	}
	if($get_sol2 > $MAX_TRANSACTION_VALUE){
		$get_sol2 = $MAX_TRANSACTION_VALUE;
	}
	foreach(@no){
...
で買い取ります。<BR>1回の取引で売り買いできるのは最大で $MAX_TRANSACTION_VALUE までです。<BR>いかほど交換致しましょうか？</font>
</TD></TR></TABLE>
</TD></TR>
<TR><TD>
