...
subtest 'left' => sub {
  my $lp = left(33)->left;
  ok $lp->isa('Either::LeftProjection');
...
subtest 'right' => sub {
  ...
  ok $rp->isa('Either::RightProjection');
...
subtest 'filter' => sub {
  ...
  ok $or->isa('Option::Some');
  $or->foreach(sub { $_->map(sub { is $_, 10 }) });
  ok $r->filter(sub { $_ < 5 })->isa('Option::None');
  my $l = left(7);
  ok $l->filter(sub { $_ > 5 })->isa('Option::None');
  ok $l->filter(sub { $_ < 5 })->isa('Option::None');
};
