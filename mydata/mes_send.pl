...
sub MES_SEND {
	...
	if(length($in{'message'}) > 3000) { &ERR("手紙は、全角１０００文字以下で入力して下さい。"); }
	&CHARA_MAIN_OPEN;
	if($in{'mes_id'} eq "$kid") { &ERR("自分には送れません。"); }
	&TOWN_DATA_OPEN($kpos);
	&COUNTRY_DATA_OPEN($kcon);

	if($xking ne $kid && $xgunshi ne $kid && $xxsub1 ne $kid){
	if(length($in{'mes_id'}) < 4 && $in{'mes_id'} ne "333" && $in{'mes_id'} ne "111" && $in{'mes_id'} ne "$kcon") { &ERR("他国に手紙を送れるのは君主と軍師と宰相だけです。。"); }
	}
