...
sub KAGO {
	...
	}elsif(($ksien ne "2")&&($ksien ne "3")&&($ksien ne "6")&&($ksien ne "7")&&($ksien ne "8")&&($ksien ne "9")){
		&ERR("加護【小】を修得していません。");
	}elsif($ksiki < 0){
		&ERR("士気が足りません。");
	}else{
...
($erenpei,$ehitokiri,$estk,$egsk,$eoujyou2,$eago,$eagoL,$eyoudou,$eyoudouL,$eobu,$eobuL,$esendou,$esendouL,$enaiskl,$ekeiskl,$etanskl,$ekinto,$e_jsub1,$e_jsub2) = split(/,/,$esub4);
	...
	$eago = $idate+$kagojikan3;
		if($eid eq $kid){$kago = $idate+$kagojikan3;}
	$kex_add = int($kagojikan3/140);

		if($eid ne $kid){
		&E_LOG("<font color=orange>【支援】</font>$knameが加護【小】を行いました。<font color=red>$kagojikan3</font>秒間自軍の防御力が上昇します。");
		&E_LOG2("<font color=orange>【支援】</font>$knameが加護【小】を行いました。<font color=red>$kagojikan3</font>秒間自軍の防御力が上昇します。");
		}

    my $log = "<font color=orange>【支援】</font>$enameに加護【小】を行いました。<font color=red>$kagojikan3</font>秒間$enameの部隊の防御力が上昇します。 士気-<font color=red>$MOR_S</font> 貢献値+<font color=red>$kex_add</font> 書物威力+<font color=red>0.02</font>";
	&K_LOG($log);
	&K_LOG2($log);
...
$battkemapname = "$TIKEI{$kiti}";
	...
	&K_LOG("<font color=orange>【支援】</font>$enameに加護【小】を行おうとしましたが失敗しました。");
	&K_LOG2("<font color=orange>【支援】</font>$enameに加護【小】を行おうとしましたが失敗しました。");
	$kkoutime = $idate+int($BMT_REMAKE/2);
	$ksenj = "$kjinkei,$ksyuppei,$kiti,$kx,$ky,$kbattletime,$kkoutime,$kidoup,$kkicn,$kksup,$kmsup,$ksakup,$konmip,";
	&CHARA_MAIN_INPUT;
				}
	...
	print <<"EOM";
<CENTER><hr size=0><h2>$enameに加護【小】を行いました。</h2><p>
