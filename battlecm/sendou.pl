...
sub SENDOU {
	...
	}elsif(($kskl2 ne "4")&&($kskl2 ne "5")&&($kskl2 ne "6")&&($kskl2 ne "7")&&($kskl2 ne "8")&&($kskl2 ne "9")){
		&ERR("扇動【小】を修得していません。");
	}elsif($ksiki < 0){
		&ERR("士気が足りません。");
	}else{
...
($erenpei,$ehitokiri,$estk,$egsk,$eoujyou2,$eago,$eagoL,$eyoudou,$eyoudouL,$eobu,$eobuL,$esendou,$esendouL,$enaiskl,$ekeiskl,$etanskl,$ekinto,$e_jsub1,$e_jsub2) = split(/,/,$esub4);
	...
	$esendou = $idate+$kagojikan3;
	$kex_add = int($kagojikan3/140);
	
  my $enemy_log = "<font color=yellowgreen>【妨害】</font>$knameが扇動【小】を行いました。<font color=red>$kagojikan3</font>秒間自軍の守備力が減少します。";
	&E_LOG($enemy_log);
	&E_LOG2($enemy_log);

  my $my_log = "<font color=yellowgreen>【妨害】</font>$enameに扇動【小】を行いました。<font color=red>$kagojikan3</font>秒間$enameの部隊の守備力が減少します。 士気-<font color=red>$MOR_S</font> 貢献値+<font color=red>$kex_add</font> 書物威力+<font color=red>0.02</font>";
	&K_LOG($my_log);
	&K_LOG2($my_log);
...
$battkemapname = "$TIKEI{$kiti}";
	...
	&K_LOG("<font color=yellowgreen>【妨害】</font>$enameに扇動【小】を行おうとしましたが失敗しました。");
	&K_LOG2("<font color=yellowgreen>【妨害】</font>$enameに扇動【小】を行おうとしましたが失敗しました。");
	$kkoutime = $idate+int($BMT_REMAKE/2);
	$ksenj = "$kjinkei,$ksyuppei,$kiti,$kx,$ky,$kbattletime,$kkoutime,$kidoup,$kkicn,$kksup,$kmsup,$ksakup,$konmip,";
	&CHARA_MAIN_INPUT;
				}
	...
	print <<"EOM";
<CENTER><hr size=0><h2>$enameに扇動【小】を行いました。</h2><p>
