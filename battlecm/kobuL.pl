...
sub KOBUL {
	...
	}elsif(($kskl2 ne "3")&&($kskl2 ne "7")&&($kskl2 ne "8")){
		&ERR("鼓舞【大】を修得していません。");
	}elsif($ksiki < 0){
		&ERR("士気が足りません。");
	}else{
...
($erenpei,$ehitokiri,$estk,$egsk,$eoujyou2,$eago,$eagoL,$eyoudou,$eyoudouL,$eobu,$eobuL,$esendou,$esendouL,$enaiskl,$ekeiskl,$etanskl,$ekinto,$e_jsub1,$e_jsub2) = split(/,/,$esub4);
	...
	$eobuL = $idate+$kagojikan3;
		if($eid eq $kid){$kobuL = $idate+$kagojikan3;}
	$kex_add = int($kagojikan3/140);

		if($eid ne $kid){
		&E_LOG("<font color=orange>【支援】</font>$knameが鼓舞【大】を行いました。<font color=red>$kagojikan3</font>秒間自軍の攻撃力が上昇します。");
		&E_LOG2("<font color=orange>【支援】</font>$knameが鼓舞【大】を行いました。<font color=red>$kagojikan3</font>秒間自軍の攻撃力が上昇します。");
		}

  my $log = "<font color=orange>【支援】</font>$enameに鼓舞【大】を行いました。<font color=red>$kagojikan3</font>秒間$enameの部隊の攻撃力が上昇します。 士気-<font color=red>$MOR_L</font> 貢献値+<font color=red>$kex_add</font> 書物威力+<font color=red>0.03</font>";
	&K_LOG($log);
	&K_LOG2($log);
...
$battkemapname = "$TIKEI{$kiti}";
	...
	&K_LOG("<font color=orange>【支援】</font>$enameに鼓舞【大】を行おうとしましたが失敗しました。");
	&K_LOG2("<font color=orange>【支援】</font>$enameに鼓舞【大】を行おうとしましたが失敗しました。");
	$kkoutime = $idate+int($BMT_REMAKE*2/3);
	$ksenj = "$kjinkei,$ksyuppei,$kiti,$kx,$ky,$kbattletime,$kkoutime,$kidoup,$kkicn,$kksup,$kmsup,$ksakup,$konmip,";
	&CHARA_MAIN_INPUT;
				}
	...
	print <<"EOM";
<CENTER><hr size=0><h2>$enameに鼓舞【大】を行いました。</h2><p>
