package Jikkoku::Class::Skill::Disturb::Stuck {
...

  {
    ...
    my %attributes = (
      ...
      max_sucess_pc        => 0.8,
      consume_morale       => 12,
      get_contribute_coef  => 0.01,
      add_book_power       => 0.05,
      min_effect_time_coef => 2.5,
      max_effect_time_coef => 3.5,
      action_interval_time => $config->{game}{action_interval_time} * 0.5,

      consume_skill_point => 10,
    );
    Class::Accessor::Lite::Lazy->mk_accessors(keys %attributes);

    my @lazy_attributes = qw/
      depend_abilities
      next_skill
    /;
    Class::Accessor::Lite::Lazy->mk_lazy_accessors(@lazy_attributes);
